...
void pengembalianBuku() {
    ...
    // Cari buku dan peminjam
    NodeBuku *buku = findBuku(daftarBuku, judul);
    NodePeminjam *peminjam = findPeminjamByNama(daftarPeminjam, nama, judul);

    if (buku == NULL || peminjam == NULL || !peminjam->sedang_meminjam) {
        printf("[ERROR] Data tidak valid atau peminjam tidak sedang meminjam buku ini.\n");
        return;
    }

    // Update stok buku
    updateStokBuku(&daftarBuku, judul, 1);
    
    // Hapus peminjam dari antrian
    deletePeminjamByNama(&daftarPeminjam, nama, judul);
    
    printf("Pengembalian buku \"%s\" oleh %s berhasil.\n", judul, nama);
    
    // Cari peminjam berikutnya
...
void pembatalanPeminjaman() {
    if (isEmptyStack(riwayatAktivitas)) {
        printf("[INFO] Tidak ada aktivitas yang dapat dibatalkan.\n");
        return;
    }
    ...
    // Hapus peminjam dari antrian
    NodePeminjam *peminjam = findPeminjamByNama(daftarPeminjam, nama, judul_buku);
    if (peminjam != NULL) {
        deletePeminjamByNama(&daftarPeminjam, nama, judul_buku);
        printf("Pembatalan peminjaman \"%s\" oleh %s berhasil.\n", judul_buku, nama);
    } else {
...
void jalankanSkenarioPengujian() {
    ...
    if (dosen1 != NULL && dosen1->sedang_meminjam) {
        // Hapus Dosen1 dari antrian
        deletePeminjamByNama(&daftarPeminjam, "Dosen1", "Buku1");
        
        // Update stok buku
        updateStokBuku(&daftarBuku, "Buku1", 1);
        printf("Pengembalian berhasil oleh: Dosen1\n");
        
        // Cari peminjam berikutnya (seharusnya Anggota1)
        NodePeminjam *nextPeminjam = findNextPeminjamForBuku(daftarPeminjam, "Buku1");
        if (nextPeminjam != NULL) {
            printf("Peminjam berikutnya: %s (%s)\n", 
                nextPeminjam->nama, 
